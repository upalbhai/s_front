...
export default function TrendingSoundsPage() {
  ...
  const fetchPage = useCallback(async (targetPage: number, append: boolean) => {
    setLoading(true);
    try {
      const res = await api.get(`/sounds?sort=trending&page=${targetPage}&limit=${limit}`);
  ...
  return (
    <div className="container" style={{ padding: '4rem 0' }}>
      <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
        <h1 className="text-gradient" style={{ fontSize: '3rem', marginBottom: '1rem' }}>Trending Sound Buttons</h1>
        <p style={{ color: 'var(--text-muted)' }}>The hottest and most played meme sounds right now.</p>
      </div>
