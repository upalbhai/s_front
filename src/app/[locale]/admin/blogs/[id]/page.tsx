            ...
            <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
              ...
              {blog?.siteViews && (
                <p className="text-xs text-slate-500">
                  Views by site:{' '}
                  {catalogSites.map((s) => `${s.siteName}: ${(blog.siteViews?.[s.siteId] || 0).toLocaleString()}`).join(' · ')}
                  {' '}(total {(blog.views || 0).toLocaleString()})
                </p>
              )}
              <div className="space-y-3">
                {catalogSites.map((site) => (
                  <label key={site.siteId} className="flex items-center gap-3 cursor-pointer group">
                    <input
                      type="checkbox"
                      checked={(formData.targetSites || []).includes(site.siteId)}
                      onChange={() => handleSiteToggle(site.siteId)}
                      className="w-4 h-4 rounded accent-indigo-600"
                    />
                    <span className="text-sm font-medium text-slate-600 dark:text-slate-300 group-hover:text-foreground transition-colors">{site.siteName}</span>
                  </label>
                ))}
              </div>
            </div>
