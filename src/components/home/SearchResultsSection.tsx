    ...
    <section className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-16 md:py-20 animate-in fade-in slide-in-from-bottom-6 duration-300">
      <div className="flex items-center gap-3 mb-8">
        ...
        <h2 className="text-3xl font-black tracking-tight text-foreground">
          {isLoading ? `Searching for "${searchQuery}"...` : `Search Results for "${searchQuery}"`}
        </h2>
      </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-3">
          ...
          <span className="text-sm font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest animate-pulse">Searching the board...</span>
        </div>
        ...
        <div className="text-center py-20 bg-slate-100 dark:bg-slate-900/50 rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-800">
          ...
          <button onClick={handleClear} className="mt-4 text-primary font-bold hover:underline cursor-pointer">Clear search</button>
        </div>
      ) : (
